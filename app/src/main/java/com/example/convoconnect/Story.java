...
import com.example.convoconnect.adapter.storyadapter;
...
public class Story extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        CircleImageView im_mystoryicon=v.findViewById(R.id.img_mystory_icon);
        im_mystoryicon.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    //overide onActivityResult to get response
    @Override
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        Uri img=data.getData();
        CircleImageView c1=getActivity().findViewById(R.id.img_mystory_icon);
        c1.setImageURI(img);
        //set selected image into database

        Random r=new Random();
        int number=r.nextInt(40000);
        FirebaseStorage.getInstance().getReference().child("convoconnect"+number).putFile(img).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                ...
                {
                    Toast.makeText(getContext(),"Hello Guys",Toast.LENGTH_LONG).show();
                }
            }
        });

        //FirebaseStorage.getInstance().getReference().child("kkkdf").putFile(img)
